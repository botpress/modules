...
export default class Upcoming extends React.Component {
  ...
  render() {
    ...
    return <ListGroupItem className={style.task} header={header}>
      <span className={style.line}>Type: <span className={style.type}>{task.schedule_type}</span></span>
      <span className={style.line}>Trigger: <span className={style.trigger}>{task.schedule}</span></span>
      <RIETextArea
        value={task.action}
